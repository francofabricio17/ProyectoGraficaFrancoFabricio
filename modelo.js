...
// Aquí definimos los nombres, posiciones y a dónde nos llevará el clic
var marcadores = [
...
function init() {
    ...
    // Cambia la ruta por la de tu canción
    audioLoader.load('assets/mieros.mp3', function(buffer) {
        sound.setBuffer(buffer);
        sound.setLoop(true);
        sound.setVolume(0.3); // Volumen al 30%
    });
    ...
    // CREACIÓN DE LOS ELEMENTOS DE TEXTO HTML PARA LOS NOMBRES
    ...
    if(etiquetasContainer) {
        marcadores.forEach(function(marcador) {
            ...
            // --- AGREGADO: Al hacer clic, abre minero.html ---
...
cargarModeloSeguro("assets/barandasplaza.glb", function(modelobaranda) {
    ...
    // ====================================================
    // 2. PRIMERA COPIA (Objetos editables individualmente)
    // ====================================================
    var grupoBarandasCopia1 = new THREE.Group();

    // Modifica la escala Z (largo) o las coordenadas directamente en cada baranda según necesites
    ...
    // Ajusta la posición general de todo este primer bloque desplazado
...
function animate() {
    ...
    // MATEMÁTICAS PARA LAS ETIQUETAS 3D -> HTML
    marcadores.forEach(function(marcador) {
        if (marcador.elemento) {
            var vec = marcador.posicion.clone();
            vec.project(camera); // Proyecta el vector 3D en la pantalla 2D de la cámara
            
            // Si vec.z es menor a 1, significa que el objeto está frente a nosotros
            if (vec.z < 1) { 
                ...
                marcador.elemento.style.display = 'block'; // Lo mostramos
            } else {
                marcador.elemento.style.display = 'none'; // Lo ocultamos si miramos al lado contrario
            }
        }
    });
...
// Reproducir el sonido al primer clic del usuario en la pantalla
